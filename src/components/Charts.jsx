...
if (chartType === "instructor") {
  const statsData = data; // data is already the stats object from backend

  const donutData = [
    { name: "Modules", value: statsData.total_modules || 0 },
    { name: "Contents", value: statsData.total_contents || 0 },
    { name: "Trainees", value: statsData.total_trainees || 0 },
  ];

  // Extract unique content titles for stacking
  const contents = [...new Set(statsData.modules.flatMap((m) => m.contents.map((c) => c.content_title)))];

  // Transform modules for stacked bar chart
  const stackedData = statsData.modules.map((m) => {
    const row = { module: m.module_title };
    m.contents.forEach((c) => {
      row[c.content_title] = c.trainee_count;
    });
    return row;
  });
  ...
  const CustomTooltip = ({ active, payload, label }) => {
    if (active && payload && payload.length) {
      return (
        <div className="custom-tooltip bg-light p-2 rounded shadow-sm" style={{ border: "1px solid #ccc", minWidth: "180px" }}>
          <p className="fw-bold mb-1 text-success">{`Module: ${label}`}</p>
          {payload.map((entry, idx) => (
            <p key={idx} className="mb-0" style={{ color: entry.color }}>
              {`${entry.name}: ${entry.value} trainee${entry.value === 1 ? "" : "s"}`}
            </p>
          ))}
        </div>
      );
    ...
    <div className="row">
      ...
      <ChartContainer title="Trainees per Module (Stacked by Content)">
        <ResponsiveContainer width="100%" height={450}>
          <BarChart data={stackedData} margin={{ top: 30, right: 30, left: 10, bottom: 50 }}>
            <CartesianGrid strokeDasharray="3 3" />
            <XAxis dataKey="module" angle={-15} textAnchor="end" interval={0} height={60} />
            <YAxis />
            <Tooltip content={<CustomTooltip />} cursor={{ fill: "rgba(0,100,0,0.1)" }} />
            <Legend />
            {contents.map((content, i) => (
              <Bar key={content} dataKey={content} stackId="a" fill={COLORS[i % COLORS.length]} radius={[4, 4, 0, 0]} />
            ))}
          </BarChart>
        </ResponsiveContainer>
      </ChartContainer>
